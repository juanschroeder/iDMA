...
module idma_desc64_axi_axis_reshaper #(
...
assign idma_req_metadata_o.do_irq = descriptor_i.flags[0];
assign idma_req_metadata_o.preserve_desc = descriptor_i.flags[30];

logic [2:0] src_protocol;
logic [2:0] dst_protocol;
assign src_protocol = idma_req_o.opt.src_protocol;
assign dst_protocol = idma_req_o.opt.dst_protocol;
...
always_comb begin
        ...
        idma_req_o.opt.beo.src_max_llen   = 3'd5;
        // this frontend currently only supports completely debursting
        idma_req_o.opt.beo.dst_max_llen   = 3'd5;
        idma_req_o.opt.beo.src_reduce_len = descriptor_i.flags[7];
        idma_req_o.opt.beo.dst_reduce_len = descriptor_i.flags[7];
        idma_req_o.opt.src_protocol       = idma_pkg::protocol_e'(descriptor_i.flags[26:24]);
        idma_req_o.opt.dst_protocol       = idma_pkg::protocol_e'(descriptor_i.flags[29:27]);
end

assign descriptor_valid_o =
    ((src_protocol == idma_pkg::AXI) &&
     ((dst_protocol == idma_pkg::AXI) ||
      (dst_protocol == idma_pkg::AXI_STREAM)));
