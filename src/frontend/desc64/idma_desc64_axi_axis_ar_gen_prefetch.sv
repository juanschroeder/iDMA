...
module idma_desc64_axi_axis_ar_gen_prefetch #(
    /// AXI Data width
    ...
    /// Reset
    input  logic         rst_ni,
    /// Clear retained chain/prefetch state after a graceful stop has drained
    input  logic         clear_i,
    /// AXI AR channel
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni || clear_i) begin
        inflight_counter_q <= '0;
        base_addr_q        <= '0;
        next_addr_valid_q  <= 1'b0;
        base_valid_q       <= 1'b0;
        flush_q            <= 1'b0;
    end else begin
        inflight_counter_q <= inflight_counter_d;
        base_addr_q        <= base_addr_d;
        next_addr_valid_q  <= next_addr_valid_d;
        base_valid_q       <= base_valid_d;
        flush_q            <= flush_d;
    end
end
